<template>
  <div class="flex flex-row justify-between mb-4 border-b border-gray-200 dark:border-gray-700">
    <ul class="flex flex-wrap -mb-px" id="myTab" data-tabs-toggle="#myTabContent" role="tablist">
      <li class="mr-2" role="presentation">
        <button
          :class="tab === 'movies' ? 'border-blue-300' : '' " 
      ...
      <li class="mr-2" role="presentation">
        <button
          :class="tab === 'favourites' ? 'border-blue-300' : '' "
          class="inline-block py-4 px-4 text-sm font-medium text-center text-gray-500 rounded-t-lg border-b-2 border-transparent hover:text-gray-600 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300"
          type="button"
          role="tab"
...
export default defineComponent({
  ...
  setup (props, { emit }) {
    ...
    function setSearch (search: Event) {
      emit('setSearch', (search.target as HTMLInputElement).value)
    }
    return {
      changeTab,
      setSearch
